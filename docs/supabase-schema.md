...
Backs the shareable public status page (`app/s/[token]/page.tsx`). One row per publish action;
unpublishing sets `revoked = true` rather than deleting, keeping an audit trail.
